...
class CameraFragment : Fragment() {
    ...
    // QR 코드 스캔을 위한 ActivityResultLauncher 선언
    val qrCodeScanLauncher =
            ...
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                if (result != null) {
                    if (result.contents == null) {
                        Toast.makeText(requireContext(), "인식된 QR-data가 없습니다.", Toast.LENGTH_LONG).show()
                    } else {
                        val qrCodeValue = result.contents.toString()
                    ...
                    Toast.makeText(requireContext(), "QR스캔에 실패했습니다.", Toast.LENGTH_LONG).show()
                }
            }
        }
    override fun onCreateView(
        ...
        fragmentCameraBinding!!.btnQr1.setOnClickListener()
        {
            ...
            qrCodeScanLauncher.launch(integrator.createScanIntent())
